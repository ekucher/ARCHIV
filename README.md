...
Loads local configuration from BRAVO.config.ps1.
Checks Administrator rights, PowerShell version, OS version, and free disk space.
Stops Apache, exchangAPI, and BRAVO services/processes when present.
Checks .md model file sizes against the configured limit.
Creates a model archive before restore when restore is active.
Runs Bravo model restore through bravocmd.exe.
Compares model file sizes after restore and rolls back from the pre-restore archive if critical shrinkage is detected.
Creates a model archive after successful restore.
Moves trace, exchangAPI, Apache, and WWW logs into archive folders.
Restarts BRAVO, exchangAPI, and Apache.
Cleans up old logs and archives according to retention settings.
Sends a final Slack report when enabled.
Optionally shuts down the server if AutoShutdown is enabled.
Exit codes
